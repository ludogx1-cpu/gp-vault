...
class _PtcEarnPageState extends State<PtcEarnPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (user == null || _userStream == null || _adsStream == null) {
      return AppScaffold(
        appBar: AppBar(
          title: const Text(
            'EARN DOGE (PTC)',
    ...
    return AppScaffold(
      ...
      body: StreamBuilder<DocumentSnapshot>(
        ...
        builder: (context, userDocSnapshot) {
          ...
          if (userDocSnapshot.hasData && userDocSnapshot.data!.exists) {
            var uData = userDocSnapshot.data!.data() as Map<String, dynamic>?;
            Map<String, dynamic>? ptcHistory =
                uData?['ptc_history'] as Map<String, dynamic>?;
            if (ptcHistory != null) {
              final now = DateTime.now();
                    ...
                    adContent = Column(
                      children: allAds.map((doc) {
                        ...
                        double reward = (adData['reward'] as num? ?? 0.002)
                            .toDouble();
                        String title =
                            adData['title'] as String? ?? "Sponsored Website";
                        String targetUrl =
                              ...
                              child: ElevatedButton.icon(
                                ...
                                onPressed: canClaimBonus
                                    ? () {
                                        launchUrl(
                                          Uri.parse(
                                            'https://golden-paw-database.web.app/sponsors.html',
                                          ),
                                        );
                                        showDialog(
                                          barrierDismissible: false,
